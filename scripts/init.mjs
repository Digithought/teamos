...
// Everything under team/ is synced except these. `.logs/` also holds
// runner-managed state (scheduler-state.json, watch observations), so the
// entries are ensured on every init rather than only written for a new file —
// an older team/.gitignore missing one would quietly sync that churn.
const TEAM_GITIGNORE_ENTRIES = ['.logs/', '.stop', '.pause'];
...
async function createTeamScaffold(projectRoot) {
	...
	// Create / top up .gitignore
	const gitignorePath = join(teamDir, '.gitignore');
	let addedEntries = 0;
	for (const entry of TEAM_GITIGNORE_ENTRIES) {
		if (await addGitignoreEntry(gitignorePath, entry)) addedEntries++;
	}
	if (addedEntries > 0) {
		log(`Updated team/.gitignore (${addedEntries} entry/entries)`);
	} else {
		log('team/.gitignore already complete, skipping');
	}
}
...
async function createTeamStubs(projectRoot) {
	...
	for (const name of AGENT_RULE_NAMES) {
		await writeIfMissing(join(teamDir, name), STUB_CONTENT, `team/${name}`);
	}
}
