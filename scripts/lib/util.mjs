...
export function buildToolsPromptSection(role) {
	...
	lines.push(
		...
		'- **clear_trigger_matches** — Mark fired commits as handled (`triggerId` and/or `hashes`). Returns `{ cleared }`.',
		'',
		'Commits authored by you are skipped by default. A match is **durable, not one-shot**: it keeps reappearing in every cycle prompt — even ones where you\'re working on something unrelated — until you call `clear_trigger_matches`. Nothing clears it for you, so review a trigger\'s fired batch and clear it in the same cycle you read it.',
		'',
