...
import { buildCleanupPrompt, heldClaims, leftoversSince, parseStatusZ, readClaims, recordClaims, releaseClaims, snapshotDirty, withoutClaimed } from './leftovers.mjs';
...
test('claims: a chat-held path is not a cycle leftover until someone changes it again', async () => {
	const dir = repo();
	const teamDir = join(dir, 'team');
	mkdirSync(teamDir);
	try {
		const before = snapshotDirty(dir, teamDir);
		writeFileSync(join(dir, 'a.txt'), 'chat edit\n');
		const chatAfter = snapshotDirty(dir, teamDir);
		await recordClaims(teamDir, 'chat:Devin:1', leftoversSince(before, chatAfter), chatAfter);
		writeFileSync(join(dir, 'b.txt'), 'cycle edit\n');
		let after = snapshotDirty(dir, teamDir);
		assert.deepEqual(withoutClaimed(leftoversSince(before, after), await readClaims(teamDir), after).map((l) => l.path), ['b.txt']);
		assert.deepEqual(heldClaims(await readClaims(teamDir), 'chat:Devin:1', after).map((l) => l.path), ['a.txt']);

		writeFileSync(join(dir, 'a.txt'), 'cycle changed it after the chat\n');
		after = snapshotDirty(dir, teamDir);
		assert.deepEqual(withoutClaimed(leftoversSince(before, after), await readClaims(teamDir), after).map((l) => l.path).sort(), ['a.txt', 'b.txt']);
		assert.deepEqual(heldClaims(await readClaims(teamDir), 'chat:Devin:1', after), []);

		await releaseClaims(teamDir, 'chat:Devin:1');
		assert.deepEqual(await readClaims(teamDir), {});
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
});

test('cleanup prompt names what ended', () => {
	assert.match(buildCleanupPrompt([{ path: 'x', status: '??' }], { ended: 'chat' }), /Your chat has ended/);
});
