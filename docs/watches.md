...
- **Acknowledged state advances on successful cycle (at-least-once).** The stored signature moves to what the agent saw only when the cycle exits 0. "What the agent saw" is the snapshot the cycle prompt was built from: a transition observed while the cycle is running is not acknowledged with it, and appears in the next prompt. A failed or killed cycle re-fires the same transition next pass.
