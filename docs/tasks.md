...
- **Blocked items are inert, not just deprioritized.** An item with `status: "blocked"` remains in the list and in the agent's prompt, but the scheduler treats it as non-work — it will *not* trigger a cycle on its own. A member whose only items are blocked goes dormant until something else arrives (a message, a due event, a new todo). Use `blocked` only when the unblock will come from outside the member (another member, an inbox message, a human). If the agent needs to periodically re-check a stalled item *itself*, leave `status` unset, demote the priority, and describe the block in `notes` / `description` so the item still counts as actionable and still drives cycles.
